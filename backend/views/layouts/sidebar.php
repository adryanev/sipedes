            ...
            <?= \common\widgets\Menu::widget(
                [
                    'items' => [
                        ...
                        ['label'=>'Pengguna', 'icon'=>'zmdi-accounts','url'=>['/user'],'template'=>$controllerName === 'user' && Yii::$app->user->identity->jabatan !='pimpinan'? '<a href="{url}" class="waves-effect active">{icon} {label}</a>': '<a href="{url}" class="waves-effect">{icon} {label}</a>'],
